 ...
 * binary skips that. The artefact is large, so it is gitignored and built on first
...
const result = await Bun.build({
  ...
  target: 'bun',
  compile: { outfile: join(root, 'bin', 'ariadne-hook') },
})
...
console.error('built bin/ariadne-hook')
